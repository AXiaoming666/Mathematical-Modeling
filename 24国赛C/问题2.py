...
if __name__ == '__main__':
    ...
    def obj_func(solution, year):
        ...
        for i in range(59):
            if i == 5 or i == 6:
                ...
                anticipate_worst[i] = anticipate[i] * 0.95

        
        all_worst = np.zeros(shape=(59))    # 每种粮食最差总生产量
        for i in range(59):
            all_worst[i] = all[i] * 0.9
        normal_worst = np.minimum(all_worst, anticipate_worst)    # 每种粮食正常出售的部分
        ...
        income_worst = ((normal_worst + (all_worst - normal_worst) * 0.5) * price_worst).sum()    # 最差收益
        profit_worst = income_worst - cost    # 最差利润

        
        objective = profit_worst    # 目标函数
    ...
    for i in range(10000):
        ...
        for j in range(7):
            ...
            for k in range(59):
                all[k] = all[k] * np.random.uniform(0.9, 1.1)
