    ...
    if index < 15:
        ...
        if index % 2 == 0:
            ...
            for i in range(50, 54):
                planting_salesprice_matrix[index] = salesprice
            for i in range(50, 54):
                planting_yield_matrix[index, i] = yield_ * excel1_sheet0.loc[i - 16, '地块面积/亩']
...
# 填充2023年种植情况矩阵
    if row['种植地块'][0] == 'A':    # 平旱地
        ...
        if row['种植季次'] == '单季':
            ...
            planting_2023_matrix[(row['作物编号'] - 1 - 16) * 2 + 17, 26 + int(row['种植地块'][1:]) - 1] = row['种植面积/亩'] / planting_area_matrix[26 + int(row['种植地块'][1:]) - 1]
        ...
        if row['种植季次'] == '第一季':
            ...
            planting_2023_matrix[row['作物编号'] - 1 + 16, 34 + int(row['种植地块'][1:]) - 1] = row['种植面积/亩'] / planting_area_matrix[34 + int(row['种植地块'][1:]) - 1]
    else:    # 智慧大棚
        if row['种植季次'] == '第一季':
